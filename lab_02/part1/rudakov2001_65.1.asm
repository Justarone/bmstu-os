; // Требуется для обарботки команд тридцатидвухразрядных процессоров
.386P				; (1)
...
; Сегмент данных // (use16 - по умолчанию будут использоваться 16битные адреса; требуется, потому что программа запускается из-под доса)
...
main	proc			; (25)
	...
	mov	ax,data		; (27) Загрузим в DS сегментный // получаем сегментный адрес!
...
; // загрузка линейного адреса и границы через псевдодескриптор
	mov	dword ptr pdescr+2,ebp		; (49) база GDT
	mov	word ptr pdescr,gdt_size-1	; (50) Граница GDT
	lgdt	pdescr				; (51) Загрузим регистр GDTR // и сообщим процессору
